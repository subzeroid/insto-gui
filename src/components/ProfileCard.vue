...
const props = defineProps<{ profile: { value: SnapshotFields | null; loading: boolean; error: DesktopFailure | null } }>()
...
const TEXTS = ['biography', 'external_url'] as const
...
const username = computed(() => (typeof known('username') === 'string' ? (known('username') as string) : null))
...
<template>
  <section v-if="profile.error || profile.loading || profile.value" class="profile-card">
    ...
    <template v-else>
      <h3 v-if="username" class="profile-user">@{{ username }}</h3>
