...
const observedState = (state: Overview['service_state']) => props.monitoringStale ? t('service.observed_stale', { state: labels[state] }) : labels[state]
const interpreterLabel = computed(() => {
  const core = facts.value?.interpreter === 'current' ? t('service.interpreter_current') : t('service.interpreter_other')
  return facts.value?.interpreterExists === false ? t('service.interpreter_missing', { core }) : core
})
...
<template>
  <div class="service-view">
    ...
    <dl class="service-facts">
      <div><dt>{{ t('service.observed') }}</dt><dd>{{ overview ? observedState(overview.service_state) : t('service.no_data') }}</dd></div>
    ...
    <dl v-if="facts" class="registration-facts">
      ...
      <div v-if="facts.registration === 'owned'"><dt>{{ t('service.interpreter') }}</dt><dd>{{ interpreterLabel }}</dd></div>
