...
import type { LookupActivity } from '../desktop/dto'
...
const props = defineProps<{ activity: LookupActivity }>()
...
const hourLabels = Array.from({ length: 24 }, (_, hour) => t('lookup.hour_label', { hour }))
...
const places = computed(() => props.activity.locations.map(term => {
  ...
  return { name: term.key, count: term.count, at: known === undefined ? null : `${formatDecimal(known.lat, 4)}, ${formatDecimal(known.lng, 4)}` }
}))
</script>
<template>
  <div class="lookup-activity">
    ...
    <template v-else>
      <section v-if="geo.geotagged > 0" class="result-block" data-block="where">
        ...
        <p v-if="geo.centroid">{{ t('lookup.centroid', { lat: formatDecimal(geo.centroid.lat, 4), lng: formatDecimal(geo.centroid.lng, 4) }) }}</p>
        ...
        <p class="fine-print">{{ t('lookup.geo_note') }}</p>
      </section>
      ...
      <section v-if="hasTimeline" class="result-block" data-block="when">
        ...
        <p class="fine-print">
          ...
          {{ t('lookup.last_post', { time: localTime(timeline.last_post_at!) }) }}
        </p>
      </section>
