...
<template>
  <section class="snapshot-history">
    ...
    <template v-else-if="state.username !== null">
      <p v-if="!state.targets.scanComplete" class="notice warning" role="status">Поиск не завершён: просмотрено {{ state.targets.scanned }} снимков. <button type="button" class="text-button" @click="history.continueSearch()">Продолжить поиск</button></p>
      <p v-if="state.targets.diagnostics > 0" class="fine-print">Часть сохранённых записей нельзя сопоставить с именем ({{ state.targets.diagnostics }}). Полнота истории не гарантируется.</p>
      ...
      <template v-if="state.targetPk !== null">
        ...
        <p v-if="state.snapshots.diagnostics > 0" class="fine-print">Нечитаемых записей: {{ state.snapshots.diagnostics }}.</p>
