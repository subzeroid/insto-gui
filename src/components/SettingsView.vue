...
const props = defineProps<{ busy: boolean; stale: boolean; configured: boolean; serviceRunning: boolean; coreVersion: string | null; buildId: string | null; replace: (token: string) => Promise<boolean>; stop: () => Promise<boolean>; openTokenPage: () => Promise<void> }>()
...
<template>
  <section class="settings-view">
    <h1>Настройки</h1>
    <div class="settings-row"><span>Доступ к HikerAPI</span><button type="button" class="text-button" data-action="replace" :disabled="busy || stale || !configured" :aria-expanded="replacing" @click="replacing = !replacing">{{ replacing ? 'Закрыть' : 'Заменить токен' }}</button></div>
