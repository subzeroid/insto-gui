...
describe('token form', () => {
  ...
  it('toggles visibility without submitting or persisting the token', async () => {
    ...
    await wrapper.get('button[aria-label="Показать токен"]').trigger('click')
  ...
  it('explains background operation and spending before first setup', () => {
    ...
    expect(wrapper.text()).toContain('закрытия окна')
    expect(wrapper.text()).toContain('лимит HikerAPI')
    expect(wrapper.get('button[type=submit]').text()).toBe('Подключить и начать')
    expect(wrapper.get('button[type=submit]').attributes('disabled')).toBeDefined()
  })
})
