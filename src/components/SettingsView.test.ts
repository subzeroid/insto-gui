...
describe('settings', () => {
  ...
  it('saves a chosen language and reloads into it, and does nothing for the current one', async () => {
    const reload = vi.fn()
    window.localStorage.removeItem('insto.locale')
    const { wrapper } = make({ reload })
    const select = wrapper.get('select[name="language"]')
    expect((select.element as HTMLSelectElement).value).toBe('en')
    expect(select.findAll('option').map(option => option.text())).toEqual(['English', 'Русский'])
    await select.setValue('en')
    expect(reload).not.toHaveBeenCalled()
    await select.setValue('ru')
    expect(window.localStorage.getItem('insto.locale')).toBe('ru')
