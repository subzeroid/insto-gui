...
artifacts_stage() {
  ...
  local runtime
  runtime="$(cd "$REPO" && python3 -B -m scripts.verify_app_runtime "$APP")"
  ...
  record hashes pass dmg_sha256 "$(shasum -a 256 "$DMG" | cut -d' ' -f1)" \
    executable_sha256 "$(shasum -a 256 "$APP/Contents/MacOS/insto-gui" | cut -d' ' -f1)"
}
...
install_stage() {
  ...
  local build_id
  build_id="$(cd "$REPO" && python3 -B -m scripts.verify_app_runtime /Applications/insto.app | jq -r .build_id)"
